...
################################################################################
idx <- which(names(counts_ls) == "SM") #Change for each cell type
...
ggsave("images/SM_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_SM<-subset(x = obj, ident = "SM", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/SM_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/SM_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_SM,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("SM Group.1 Heatmap (20,000 cells)")
ggsave("images/SM_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
