...
################################################################################
idx <- which(names(counts_ls) == "ADIPO") #Change for each cell type
...
ggsave("images/ADIPO_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_ADIPO<-subset(x = obj, ident = "ADIPO", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/ADIPO_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/ADIPO_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_ADIPO,                                                                   #Modify Here to features
  features = c("Ghr", "Malat1"),
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  group.colors = NULL,
  ...
  combine = TRUE) + ggtitle("ADIPO Group.1 Heatmap (20,000 cells)")
ggsave("images/ADIPO_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
