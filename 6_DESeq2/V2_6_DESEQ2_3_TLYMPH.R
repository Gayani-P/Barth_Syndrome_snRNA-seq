...
################################################################################
idx <- which(names(counts_ls) == "TLYMPH") #Change for each cell type
...
ggsave("images/TLYMPH_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_TLYMPH<-subset(x = obj, ident = "TLYMPH", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/TLYMPH_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/TLYMPH_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_TLYMPH,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("TLYMPH Group.1 Heatmap (20,000 cells)")
ggsave("images/TLYMPH_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
