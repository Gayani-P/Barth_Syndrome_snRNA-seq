...
################################################################################
idx <- which(names(counts_ls) == "EC") #Change for each cell type
...
ggsave("images/EC_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_EC<-subset(x = obj, ident = "EC", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/EC_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/EC_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_EC,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("EC Group.1 Heatmap (20,000 cells)")
ggsave("images/EC_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
