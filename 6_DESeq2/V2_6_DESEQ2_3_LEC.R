...
################################################################################
idx <- which(names(counts_ls) == "LEC") #Change for each cell type
...
ggsave("images/LEC_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_LEC<-subset(x = obj, ident = "LEC", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/LEC_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/LEC_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_LEC,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("LEC Group.1 Heatmap (20,000 cells)")
ggsave("images/LEC_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
