...
################################################################################
idx <- which(names(counts_ls) == "BLYMPH") #Change for each cell type
...
ggsave("images/BLYMPH_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_BLYMPH<-subset(x = obj, ident = "BLYMPH", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/BLYMPH_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/BLYMPH_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_BLYMPH,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("BLYMPH Group.1 Heatmap (20,000 cells)")
ggsave("images/BLYMPH_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
