...
################################################################################
#CM/FB/EC/PC/SM/MAC/LEC/NEURO/TLYMPH/DC/ADIPO
################################################################################
idx <- which(names(counts_ls) == "FB") #Change for each cell type
...
ggsave("images/FB_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_FB<-subset(x = obj, ident = "FB", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/FB_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/FB_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_FB,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("FB Group.1 Heatmap (20,000 cells)")
ggsave("images/FB_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
VlnPlot(obj_FB, features = c("Ghr", "Malat1"), group.by = "genotype")
