...
################################################################################
idx <- which(names(counts_ls) == "CM") #Change for each cell type
...
DESeq2::plotPCA(rld, ntop = 500, intgroup = "batch") #batch and genotype are confounded
...
clusters <- degPatterns(cluster_rlog, metadata = cluster_metadata, time = "genotype", col = NULL, minc = 1)
ggsave("images/CM_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_CM<-subset(x = obj, ident = "CM", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/CM_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/CM_group2_BP_sigGOTerms.csv")
################################################################################
...
DoHeatmap(
  ...
  combine = TRUE) + ggtitle("CM Group.1 Heatmap (20,000 cells)")
ggsave("images/CM_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
