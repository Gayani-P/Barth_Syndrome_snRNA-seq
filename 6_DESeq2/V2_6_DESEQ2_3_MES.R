...
################################################################################
idx <- which(names(counts_ls) == "MES") #Change for each cell type
...
ggsave("images/MES_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_MES<-subset(x = obj, ident = "MES", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/MES_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/MES_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_MES,                                                                   #Modify Here to features
  ...
  combine = TRUE) + ggtitle("MES Group.1 Heatmap (20,000 cells)")
ggsave("images/MES_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
