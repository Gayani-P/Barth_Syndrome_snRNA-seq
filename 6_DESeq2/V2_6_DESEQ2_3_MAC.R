...
################################################################################
idx <- which(names(counts_ls) == "MAC") #Change for each cell type
...
ggsave("images/MAC_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_MAC<-subset(x = obj, ident = "MAC", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/MAC_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/MAC_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_MAC,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("MAC Group.1 Heatmap (20,000 cells)")
ggsave("images/MAC_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
