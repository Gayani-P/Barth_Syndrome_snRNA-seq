...
################################################################################
idx <- which(names(counts_ls) == "DC") #Change for each cell type
...
ggsave("images/DC_LRT_direction.of.expression.png", units = "px", width = 4500, height =2970)
...
################################################################################
obj_DC<-subset(x = obj, ident = "DC", downsample = 20000) #Modify here
#Gene Universe
...
write.csv(result, "results/GO_Terms/DC_group1_BP_sigGOTerms.csv")
...
write.csv(result, "results/GO_Terms/DC_group2_BP_sigGOTerms.csv")
################################################################################
...
#Heatmap
DoHeatmap(
  obj_DC,                                                                   #Modify Here to features
  features = features,
  cells = NULL,
  group.by = "genotype",
  group.bar = TRUE,
  ...
  combine = TRUE) + ggtitle("DC Group.1 Heatmap (20,000 cells)")
ggsave("images/DC_group.1_heatmap.png", units = "px", width = 4500, height =2970)
###############################################################################
